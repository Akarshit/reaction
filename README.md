...
[Mailchimp Open Commerce](http://reactioncommerce.com) is a headless commerce platform built using Node.js, React, and GraphQL. It plays nicely with npm, Docker and Kubernetes.
...
docker-compose up -d # starts a MongoDB container and a api-server container
docker-compose logs -f api-server # view api-server container logs
```

To stop the api-server and the MongoDB server, enter `docker-compose down`.
...
```sh
dc up -d mongo
docker run --env-file ./.env -p 3000:3000 --network opencommerce.localhost -it test-api:latest
```
...
- [Gitter chat](https://gitter.im/mailchimp-open-commerce/api-server)
...
Find a bug, a typo, or something that’s not documented well? We’d love for you to [open an issue](https://github.com/mailchimp-open-commerce/api-server/issues) telling us what we can improve!
...
We love your pull requests! Check our our [`Good First Issue`](https://github.com/mailchimp-open-commerce/api-server/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22) and [`Help Wanted`](https://github.com/mailchimp-open-commerce/api-server/issues?q=label%3A%22help+wanted%22) tags for good issues to tackle.

### Pull Request guidelines
Pull requests should pass all automated tests, style, and security checks.

Your code should pass all [acceptance tests and unit tests](https://docs.reactioncommerce.com/reaction-docs/trunk/testing-reaction). Run `docker-compose run --rm api-server npm run test` to run the test suites in containers. If you're adding functionality to Mailchimp Open Commerce, you should add tests for the added functionality.

We require that all code contributed to Mailchimp Open Commerce follows [Mailchimp Open Commerce's ESLint rules](https://github.com/mailchimp-open-commerce/eslint-config). You can run `docker-compose run --rm api-server npm run lint` to run ESLint against your code locally.
